...
class EndOfTrack(Exception):
    pass

class BMS_Track(object):
    def __init__(self, fileobj = None, midiOutput = None):
    ...
    def parseTrack(self, 
                   ...
                   noteOff_callback = None,
                   terminateTrack_callback = None,
                   any_event_callback = None):
        
        bmsfile = self.bmsfile
        read = self.read
        
        bmsfile.seek(trackOffset)
        ...
        while True:
            ...
            """if value in callbacks:
                callbacks[value](self, offset, bmsfile, read)
                bmsfile.seek(currPos)"""
            ...
            if value <= 0x7F: 
                ...
                if noteOn_callback != None:
                    endPosition = int(bmsfile.tell())
                    noteOn_callback(self, trackNum, trackOffset, 
                                     valuePos, endPosition, bmsfile,
                                     *args)
                ...
                if controlEventID == 0x80: # Delay, up to 0xFF (255) ticks
                    ...
                    if noteOff_callback != None:
                        noteOff_callback(self, trackNum, trackOffset, 
                                         valuePos, endPosition, bmsfile,
                                         *args)
                ...
                #"""
                    ...
                    if terminateTrack_callback != None:
                        endPosition = int(bmsfile.tell())
                        terminateTrack_callback(self, trackNum, trackOffset, 
                                                valuePos, endPosition, bmsfile)
                ...
                endPosition = int(bmsfile.tell())
                #print "Parsed controller event {0} at pos={1}".format(hex(controlEventID),
                #                                                      hex(valuePos))
                
                if value in callbacks:
                    if args != None:
                        callbacks[value](self, trackNum, trackOffset, 
                                         valuePos, endPosition, bmsfile,
                                         *args)
                    else:
                        callbacks[value](self, trackNum, trackOffset, 
                ...
                if any_event_callback != None:
                    if args != None:
                        any_event_callback( self, trackNum, trackOffset, 
                                            valuePos, endPosition, bmsfile,
                                            value, *args)
                    else:
                        any_event_callback( self, trackNum, trackOffset, 
...
# BMSAnalyzer is an object for collecting
# data from a BMS file without filling the
# main BMS_Track object with data that is not
# necessary for playback.
# Its functions can be passed on as callback functions 
# in the parseTrack function of the BMS_Track object.
class BMSAnalyzer(object):
    def __init__(self):
        self.tracks = []
        self.track_metadata = []
    ...
    def getTracks(self, bms_object, trackNum, trackOffset, 
                  commandPos, endPosition, bmsfile, 
                  newTrackNum, newTrackOffset):
        
        self.tracks.append((newTrackNum, newTrackOffset))
    
    def getTrackLength_endOfTrack(self, bms_object, trackNum, trackOffset, 
    ...
    def getEventStatistics(self, bms_object, trackNum, trackOffset, 
                                commandPos, endPos, bmsfile, eventID, *args):
        
        if eventID not in self.event_statistics:
            self.event_statistics[eventID] = {"count" : 1,
                                              "args" : [args]}
        else:
            self.event_statistics[eventID]["count"] += 1
            self.event_statistics[eventID]["args"].append(args)
...
if __name__ == "__main__":
    ...
    def parseFile(filehandle, midiOutput):
        bmsAnalyzer = BMSAnalyzer()
        
        bms = BMS_Track(f, midiOutput = midiOutput)
        
        try:
            bms.parseTrack(callbacks = {0xC1 : bmsAnalyzer.getTracks},
                           terminateTrack_callback = bmsAnalyzer.getTrackLength_endOfTrack,
                           any_event_callback = bmsAnalyzer.getEventStatistics)
        except EndOfTrack:
            pass
        
        
        
        for trackInfo in bmsAnalyzer.tracks:
            trackNum, trackOffset = trackInfo
            
            print "Parsing track {0} at offset {1:x}".format(trackNum, trackOffset)
            
            try:
                bms.parseTrack(trackNum = trackNum,
                               trackOffset = trackOffset,
                               callbacks = {0xC1 : bmsAnalyzer.getTracks},
                               terminateTrack_callback = bmsAnalyzer.getTrackLength_endOfTrack,
                               any_event_callback = bmsAnalyzer.getEventStatistics)
            except EndOfTrack:
                pass
    ...
    with open("bms_report.txt", "w") as f:
        f.write("")
    report = open("bms_report.txt", "a")
    
    for filename in files:
        if ".bms" not in filename:
            continue
        ...
        try:
            with open(path, "rb") as f:
                bmsAnalyze = parseFile(f, midiOutput)
        except Exception as error:
            report.write(str(error)+"\n")
        else:
            trackAmount = len(bmsAnalyze.tracks)
            report.write("All done! At least {0} tracks found\n".format(trackAmount))
            
            trackInfoString_list = []
            for trackData in sorted(bmsAnalyze.track_metadata, key = lambda x:x[0]):
                trackNum, trackOffset, trackLength = trackData
                
                infoString = ("Track Nr. {0} (offset: {1}, length: {2} "
                              "bytes)".format(trackNum,trackOffset,trackLength))
                trackInfoString_list.append(infoString)
            
            report.write("Track Info: {0}\n".format(", ".join(trackInfoString_list)))
                
            report.write("Events used: \n")
            
            for event in sorted(bmsAnalyze.event_statistics.keys()):
                eventData = bmsAnalyze.event_statistics[event]
                event_msg = "{count} of 0x{eventID:x}\n".format(count = eventData["count"],
                                                          eventID = event)
                
                report.write(event_msg)
